...
fn kgraph_db() -> Result<Connection, String> {
    ...
    conn.execute_batch(KGRAPH_SCHEMA)
        .map_err(|e| e.to_string())?;
    Ok(conn)
}
...
pub fn kgraph_add_entity(name: &str, entity_type: &str, properties: &str, id: &str) -> Value {
    let valid_types = [
        "project", "file", "tool", "pattern", "error", "person", "concept", "mcp", "library",
        "platform", "config", "service", "database",
    ];
    if !valid_types.contains(&entity_type) {
        return json!({"error": format!("Invalid type: {}. Valid: {:?}", entity_type, valid_types)});
    }
    let conn = match kgraph_db() {
        Ok(c) => c,
        Err(e) => return json!({"error": e}),
    };
    let entity_id = if id.is_empty() {
        make_id(name, entity_type)
    } else {
        id.to_string()
    };
    let now = now_iso();
    ...
    if exists {
        let _ = conn.execute(
            ...
            params![properties, now, entity_id],
        );
    } else {
...
pub fn kgraph_add_edge(
    ...
    let src = match resolve_entity(&conn, source) {
        Some(s) => s,
        None => return json!({"error": format!("Entity not found: {}", source)}),
    };
    let tgt = match resolve_entity(&conn, target) {
        Some(t) => t,
        None => return json!({"error": format!("Entity not found: {}", target)}),
    };
...
pub fn kgraph_observe(
    ...
    let src = resolve_entity(&conn, source).unwrap_or_else(|| {
        ...
        let _ = conn.execute(
            "INSERT OR IGNORE INTO entities (id, name, type) VALUES (?1,?2,'concept')",
            params![id, source],
        );
        id
    });
    ...
    let (edge_id, old_weight) = match edge {
        ...
        None => {
            let _ = conn.execute(
                "INSERT INTO edges (source_id, target_id, relation, weight) VALUES (?1,?2,?3,1.0)",
                params![src, tgt, relation],
            );
            conn.query_row(
                "SELECT id, weight FROM edges WHERE source_id=?1 AND target_id=?2 AND relation=?3",
    ...
    let _ = conn.execute(
        "INSERT INTO observations (edge_id, session_id, observation, impact) VALUES (?1,?2,?3,?4)",
...
pub fn kgraph_query_neighbors(
    ...
    let start_id = match resolve_entity(&conn, entity) {
        Some(s) => s,
        None => return json!({"error": format!("Entity not found: {}", entity)}),
    };
    ...
    while let Some((node, d)) = queue.pop_front() {
        ...
        let rows: Vec<String> = if !relation.is_empty() {
            stmt.query_map(params![node, relation], |r| r.get(0))
                .unwrap()
                .flatten()
                .collect()
        } else {
            stmt.query_map(params![node], |r| r.get(0))
                .unwrap()
                .flatten()
                .collect()
        };
    ...
    let esql = format!(
        "SELECT * FROM edges WHERE source_id IN ({0}) AND target_id IN ({0})",
        placeholders
    );
...
pub fn kgraph_query_path(source: &str, target: &str, max_depth: u32) -> Value {
    ...
    while let Some((current, d)) = queue.pop_front() {
        ...
        let edges: Vec<(i64, String, String, String, f64)> = stmt
...
pub fn kgraph_delete(
    ...
    if !entity_id.is_empty() {
        let eid = match resolve_entity(&conn, entity_id) {
            Some(e) => e,
            None => return json!({"error": format!("Entity not found: {}", entity_id)}),
        };
    ...
    if !edge_source.is_empty() && !edge_target.is_empty() {
        let src = match resolve_entity(&conn, edge_source) {
            Some(s) => s,
            None => return json!({"error": "Source not found"}),
        };
        let tgt = match resolve_entity(&conn, edge_target) {
            Some(t) => t,
            None => return json!({"error": "Target not found"}),
        };
        if !edge_relation.is_empty() {
            let _ = conn.execute(
                "DELETE FROM edges WHERE source_id=?1 AND target_id=?2 AND relation=?3",
                params![src, tgt, edge_relation],
            );
        } else {
            let _ = conn.execute(
...
fn replay_db() -> Result<Connection, String> {
    ...
    .map_err(|e| e.to_string())?;
    conn.execute_batch(REPLAY_SCHEMA)
        .map_err(|e| e.to_string())?;
    Ok(conn)
}
...
fn discover_sessions(project_filter: &str) -> Vec<(String, String, u64, f64)> {
    ...
    if let Ok(entries) = std::fs::read_dir(&sessions_dir) {
        for entry in entries.flatten() {
            ...
            if !project_filter.is_empty() && !project_path.contains(project_filter) {
                continue;
            }
...
fn parse_session_jsonl(path: &str, max_messages: usize) -> Value {
    ...
    for line in reader.lines() {
        ...
        if meta["session_id"].as_str() == Some("") {
            if let Some(sid) = entry["sessionId"].as_str() {
                meta["session_id"] = json!(sid);
            }
        }
        if meta["project_path"].as_str() == Some("") {
            if let Some(cwd) = entry["cwd"].as_str() {
                meta["project_path"] = json!(cwd);
            }
        }
        if meta["started_at"].as_str() == Some("") && !ts.is_empty() {
            meta["started_at"] = json!(&ts);
        }
        if let Some(br) = entry["gitBranch"].as_str() {
            meta["git_branch"] = json!(br);
        }
        if entry_type == "summary" {
            meta["title"] = json!(entry["summary"].as_str().unwrap_or(""));
            continue;
        }
        if entry_type == "system" || entry_type == "progress" {
            continue;
        }
        ...
        if entry_type == "assistant" {
            ...
            if let Some(content) = msg["content"].as_array() {
                for block in content {
                    match block["type"].as_str().unwrap_or("") {
                        "text" => {
                            if let Some(arr) = am["text"].as_array_mut() {
                                arr.push(json!(block["text"].as_str().unwrap_or("")));
                            }
                        }
                        "tool_use" => {
                            if let Some(arr) = am["tool_calls"].as_array_mut() {
                                let inp = &block["input"];
            ...
            if msg["stop_reason"].is_string() {
                if let Some(am) = assistant_msgs.remove(&msg_id) {
                    turns.push(am);
                }
            }
            ...
            if let Some(arr) = content.as_array() {
                ...
                if !text.trim().is_empty() {
                    turns.push(
                        json!({"role":"user","text":text,"timestamp":&ts,"line_number":line_num}),
                    );
                }
            }
        }
        if max_messages > 0 && turns.len() >= max_messages {
            break;
        }
    }
    ...
    let tc_count: usize = turns
        .iter()
        .filter(|t| t["role"].as_str() == Some("assistant"))
        .map(|t| t["tool_calls"].as_array().map_or(0, |a| a.len()))
        .sum();
    let err_count = turns
        .iter()
        .filter(|t| t["is_error"].as_bool().unwrap_or(false))
        .count();
                ...
                if role == "assistant" {
                    if let Some(tcs) = turn["tool_calls"].as_array() {
                        for tc in tcs {
                            ...
                            let preview: String =
                                format!("tool:{}", tool_name).chars().take(500).collect();
                            let mid = gen_id("msg");
                    ...
                    let preview: String = turn["content"]
                        .as_str()
                        .unwrap_or("")
                        .chars()
                        .take(500)
                        .collect();
                    let is_err = if turn["is_error"].as_bool().unwrap_or(false) {
                        1
                    } else {
                        0
                    };
...
pub fn replay_search(query: &str, project: &str, tool: &str, limit: u32, days: u32) -> Value {
    ...
    if !tool.is_empty() {
        param_count += 1;
        sql += &format!(" AND m.tool_name = ?{}", param_count);
        params_vec.push(Box::new(tool.to_string()));
    }
    if !project.is_empty() {
        param_count += 1;
        sql += &format!(" AND s.project_path LIKE ?{}", param_count);
        params_vec.push(Box::new(format!("%{}%", project)));
    }
    if days > 0 {
        param_count += 1;
        sql += &format!(" AND m.timestamp >= ?{}", param_count);
        params_vec.push(Box::new(days_ago(days)));
    }
    param_count += 1;
    sql += &format!(" ORDER BY m.timestamp DESC LIMIT ?{}", param_count);
...
pub fn replay_session(
    ...
    let file_path: Option<String> = conn
        .query_row(
            "SELECT file_path FROM sessions WHERE file_path=?1 OR session_id=?1",
            params![session_id],
            |r| r.get(0),
        )
        .ok();
    ...
    if !std::path::Path::new(&fp).exists() {
        return json!({"error": format!("Session file not found: {}", fp)});
    }
    let mut session = parse_session_jsonl(&fp, max_messages as usize);
    if let Some(turns) = session["turns"].as_array_mut() {
        if !include_tools {
            turns.retain(|t| t["role"].as_str() != Some("tool_result"));
        }
        if !include_errors {
            turns.retain(|t| !t["is_error"].as_bool().unwrap_or(false));
        }
    }
    session
}
...
pub fn replay_tool_history(tool_name: &str, limit: u32, days: u32) -> Value {
    ...
    let mut params_vec: Vec<Box<dyn rusqlite::types::ToSql>> =
        vec![Box::new(tool_name.to_string())];
    if days > 0 {
        sql += " AND m.timestamp >= ?2";
        params_vec.push(Box::new(days_ago(days)));
    }
...
pub fn replay_status() -> Value {
    ...
    let error_count: i64 = conn
        .query_row("SELECT COUNT(*) FROM messages WHERE is_error=1", [], |r| {
            r.get(0)
        })
        .unwrap_or(0);
    let latest: String = conn
        .query_row("SELECT MAX(indexed_at) FROM sessions", [], |r| r.get(0))
        .unwrap_or_default();
...
pub fn fact_add(
    ...
    let exists: bool = conn
        .query_row(
            "SELECT COUNT(*) FROM facts WHERE category=?1 AND key=?2",
            params![category, key],
            |r| r.get::<_, i64>(0),
        )
        .unwrap_or(0)
        > 0;
    if exists {
        return json!({"error": format!("Fact '{}:{}' already exists. Use fact_update.", category, key)});
    }
...
pub fn fact_get(fact_id: &str, key: &str, category: &str) -> Value {
        ...
        .is_empty()
    {
        ...
        conn.query_row(
            "SELECT id,category,key,value,confidence,source,aliases,tags FROM facts WHERE key=?1",
            params![key],
            |r| {
                Ok((
                    r.get(0)?,
...
pub fn fact_check(claim: &str) -> Value {
    ...
    for entity in &entities {
        ...
        let facts: Vec<(String, String, String, String, f64)> = stmt
            .query_map(params![ev, like], |r| {
                Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?))
            })
            .unwrap()
            .flatten()
    ...
    let contradictions = results
        .iter()
        .filter(|r| r["verdict"].as_str() == Some("contradiction"))
        .count();
    let matches = results
        .iter()
        .filter(|r| r["verdict"].as_str() == Some("match"))
        .count();
...
pub fn fact_check_response(response_text: &str) -> Value {
    ...
    for sentence in &sentences {
        ...
        total_matches += m;
        if c > 0 {
            flagged.push(json!({"sentence": &sentence[..sentence.len().min(120)], "contradictions": c, "details": result["results"]}));
        }
    }
...
pub fn fact_update(
    ...
    let row: Option<String> = if !fact_id.is_empty() {
        conn.query_row("SELECT id FROM facts WHERE id=?1", params![fact_id], |r| {
            r.get(0)
        })
        .ok()
        ...
        conn.query_row("SELECT id FROM facts WHERE key=?1", params![key], |r| {
            r.get(0)
        })
        .ok()
    } else {
    ...
    let fid = match row {
        Some(id) => id,
        None => return json!({"error": "Fact not found"}),
    };
    ...
    params_vec.push(Box::new(now_iso()));
...
pub fn fact_delete(fact_id: &str, reason: &str) -> Value {
    ...
    let exists: bool = conn
        ...
        > 0;
    if !exists {
        return json!({"error": format!("Fact not found: {}", fact_id)});
    }
...
pub fn truthguard_status() -> Value {
    ...
    let total_checks: i64 = conn
        .query_row("SELECT COUNT(*) FROM fact_checks", [], |r| r.get(0))
        .unwrap_or(0);
    let contradictions: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM fact_checks WHERE verdict='contradiction'",
            [],
            |r| r.get(0),
        )
        .unwrap_or(0);
